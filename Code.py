...
cursor = db.cursor()
cursor.execute("SELECT * FROM Country INNER JOIN Medals ON Country.country_code = Medals.country_code ORDER BY Medals.gold DESC, Medals.silver DESC, Medals.bronze DESC;")
...
def add_country():
    ...
    if not country_code.isalpha() or len(country_code) != 3 or not all(part.isalpha() for part in country_name.split()):
...
while True:
    ...
    if choice == 1:
        ...
        cursor.execute("SELECT * FROM Country, Medals WHERE Country.country_code = ?;", (country_code,))
        country = cursor.fetchone()
        if country is not None:
            print(f"{'Code':<5}{'Country':<23}{'Gold':<6}{'Silver':<7}{'Bronze':<6}")
            print(f"{country[0]:<5}{country[1]:<23}{country[3]:<6}{country[4]:<7}{country[5]:<6}")
        else:
            print(f"No country found with code {country_code}.")
    elif choice == 5:   
